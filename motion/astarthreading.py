...
class AStarThread(threading.Thread):
    def __init__(self, open_set, close_set, goal_node, road_map, sample_x, sample_y, heuristic):
    ...
    def run(self):
        while self.open_set:
            current_id = min(self.open_set, key=lambda o: self.open_set[o].cost + self.heuristic(self.open_set[o], self.goal_node))
            current_node = self.open_set[current_id]

            if current_id == len(self.road_map) - 1:
                self.goal_node.cost = current_node.cost
                self.goal_node.parent = current_node.parent
                break

            del self.open_set[current_id]
            self.close_set[current_id] = current_node

            for neighbor_id in self.road_map[current_id]:
                neighbor_node = Node(self.sample_x[neighbor_id], self.sample_y[neighbor_id], 0.0, current_id)
                neighbor_node.cost = current_node.cost + math.hypot(current_node.x - neighbor_node.x, current_node.y - neighbor_node.y)

                if neighbor_id in self.close_set:
                    continue

                if neighbor_id in self.open_set:
                    if self.open_set[neighbor_id].cost <= neighbor_node.cost:
                        continue

                self.open_set[neighbor_id] = neighbor_node

class AStarthreading(object):
    ...
    def astar_search(self, start_x, start_y, goal_x, goal_y, sample_x, sample_y, road_map):
        ...
        while open_set:
            threads = []
            for _ in range(4):  # Assuming 4 threads
                thread = AStarThread(open_set, close_set, goal_node, road_map, sample_x, sample_y, self.heuristic)
                thread.start()
